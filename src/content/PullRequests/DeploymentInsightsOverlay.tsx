...
const SUPABASE_MERGED_PRS_WORKFLOW_ID = 'SUPABASE_MERGED_PRS';

function prToDeployment(pr: PR): Deployment {
  return {
    id: pr.id,
    status: 'SUCCESS',
    head_branch: pr.head_branch ?? '',
    event_actor: pr.author,
    created_at: pr.created_at,
    updated_at: pr.updated_at,
    conducted_at: pr.state_changed_at ?? pr.updated_at,
    pr_count: 1,
    run_duration: 0,
    html_url: pr.pr_link ?? '',
    repo_workflow_id: SUPABASE_MERGED_PRS_WORKFLOW_ID
  };
}
...
export const DeploymentInsightsOverlay = () => {
  ...
  const teamReposFromTeamRepos = useSelector((s) =>
    singleTeamId ? s.team.teamReposMaps?.[singleTeamId] ?? [] : []
  );
  const summaryPrs = useSelector((s) => s.doraMetrics.summary_prs ?? []);

  const deploymentsListByRepo = useMemo(() => {
    const fromDeployments = sort(
      descend((r: { repo: { id: string }; deps: Deployment[] }) => r.deps.length),
    ...
    return (teamReposFromTeamRepos || []).map((r) => {
      const deps = summaryPrs
        .filter((pr) => pr.repo_name === r.name)
        .map(prToDeployment);
      return {
        repo: { id: r.id, name: r.name, org_name: r.org_name ?? '' } as BaseRepo,
        deps
      };
    });
  }, [
    teamDeployments.deployments_map,
    teamDeployments.repos_map,
    teamReposFromTeamRepos,
    summaryPrs
  ]);
  ...
  useEffect(() => {
    if (!deploymentsListByRepo.length) return;
    const selectedInList = deploymentsListByRepo.some(
      (r) => r.repo?.id === selectedRepo.value
    );
    if (selectedRepo.value && selectedInList) return;
  ...
  const deployments = useMemo(() => {
    ...
    const fromMap = teamDeployments.deployments_map?.[selectedRepo.value];
    if (fromMap?.length) return fromMap;
    return (
      deploymentsListByRepo.find((r) => r.repo?.id === selectedRepo.value)
        ?.deps ?? []
    );
  }, [
    selectedRepo.value,
  ...
  const prsToShow = useMemo(() => {
    if (!selectedDepID.value) return [];
    const prFromSummary = summaryPrs.find((pr) => pr.id === selectedDepID.value);
    if (prFromSummary) return [prFromSummary];
    return statePrs;
  }, [selectedDepID.value, summaryPrs, statePrs]);

  const {
    sortedList: prs,
    updateSortConf,
    conf
  } = useTableSort(prsToShow, { field: 'lead_time', order: 'desc' });

  const selectDeployment = useCallback(
    (dep: Deployment) => {
      selectedDepID.set(dep.id);
      const isSupabaseDeployment = summaryPrs.some((pr) => pr.id === dep.id);
      if (!isSupabaseDeployment) {
        dispatch(fetchDeploymentPRs({ deployment_id: dep.id }));
      }
    },
    [dispatch, selectedDepID, summaryPrs]
  );
  ...
  const fallbackWorkflow: RepoWorkflowExtended = useMemo(
    () => ({
      id: SUPABASE_MERGED_PRS_WORKFLOW_ID,
      name: 'Merged PRs',
  ...
  const groupedDeployments: GroupedDeployments[] = useMemo(() => {
    ...
    const workflowDeploymentTuples = groupedDeploymentsEntries.map(
      ([workflowId, deployments]) =>
        [
          workflowsMap[workflowId] ?? (workflowId === SUPABASE_MERGED_PRS_WORKFLOW_ID ? fallbackWorkflow : {}),
          deployments
  ...
  }, [filteredDeployments, workflowsMap, fallbackWorkflow]);
