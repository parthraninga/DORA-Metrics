  ...
  return (
    <FlexBox col gap={2}>
      <FlexBox col>
        ...
        <Line>
          {useSupabaseTeams
            ? 'Select repositories from your synced repos (Sync Repos)'
            : 'Select repositories for this team using name or link'}
        </Line>
      </FlexBox>
      <Box display="flex" alignItems="center" gap={2}>
        <Autocomplete
          noOptionsText={
            useSupabaseTeams
              ? repoOptions.length === 0
                ? 'No repos in database. Add repos via Sync Repos first.'
          ...
          renderOption={(props, option, { selected }) => {
            const orgName = option.parent || (option as any).org_name || '';
            return (
              <li {...props}>
                <FlexBox
                  gap={2}
                ...
                >
                  ...
                  >
                    <FlexBox gap={1} alignCenter>
                      <Avatar
                        src={orgName ? `https://github.com/${orgName}.png?size=20` : undefined}
                        sx={{ width: 20, height: 20 }}
                      >
                        {!orgName ? (option.provider === Integration.GITHUB ? <GitHub sx={{ fontSize: 12 }} /> : <GitlabIcon height={10} width={10} />) : null}
                      </Avatar>
                      <Line tiny>
        ...
        <Table size="small">
          ...
          <TableBody>
            {paged.map((repo) => {
              ...
              const orgName = repo.parent || (repo as any).org_name || '';
              return (
                <TableRow key={repo.id}>
                  <TableCell sx={{ px: 2 }}>
                    ...
                    >
                      <Avatar
                        src={orgName ? `https://github.com/${orgName}.png?size=24` : undefined}
                        sx={{ width: 24, height: 24 }}
                      >
                        {!orgName ? (repo.provider === Integration.GITHUB ? <GitHub fontSize="small" /> : <GitlabIcon height={14} width={14} />) : null}
                      </Avatar>
                      {name}
                    </FlexBox>
                  </TableCell>
