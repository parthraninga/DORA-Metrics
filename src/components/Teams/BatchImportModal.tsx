...
export const BatchImportModal: FC<BatchImportModalProps> = ({
  ...
  const [repoType, setRepoType] = useState<'org' | 'personal'>('org');
  const [orgName, setOrgName] = useState<string>('');
  const [personalRepoSearch, setPersonalRepoSearch] = useState<string>('');
  ...
  const fetchPage = async (pageNum: number) => {
    ...
    try {
      ...
      // Apply search filter if one exists for personal repos
      if (repoType === 'personal' && personalRepoSearch.trim()) {
        const lower = personalRepoSearch.toLowerCase();
        setFiltered(
          repos.filter((r: BaseRepo) =>
  ...
  const fetchAll = async (): Promise<BaseRepo[]> => {
    setLoadingPage(true);
    try {
      const params: any = { provider, repo_type: repoType, select_all: true };
      if (repoType === 'org') {
        params.org = orgName;
      }
      const resp = await axios.get(`/api/internal/${orgId}/git_org_repos`, {
  ...
  const handleFilter = (q: string) => {
    ...
    if (repoType === 'personal') {
      // For personal repos, filter across all loaded pages
      const allLoadedRepos: BaseRepo[] = [];
      Object.values(pages).forEach((pageData) => {
        allLoadedRepos.push(...pageData.repos);
      });
      setFiltered(
        allLoadedRepos.filter((r) =>
          `${r.parent}/${r.name}`.toLowerCase().includes(lower)
        )
      );
    } else {
      // For org repos, filter current page only
      ...
      <Box display="flex" gap={2} flexWrap="wrap">
        <FormControl sx={{ minWidth: 140 }}>
          ...
          <Select
            ...
            label="Provider"
            onChange={(e) => {
              setProvider(e.target.value as Integration);
              setPages({});
              setFiltered([]);
            }}
          >
        ...
        <ToggleButtonGroup
          ...
          onChange={(_, newValue) => {
            if (newValue !== null) {
              setRepoType(newValue);
              setPages({});
              setFiltered([]);
              setOrgName('');
              setPersonalRepoSearch('');
            }
          }}
          aria-label="repository type"
        >
          <ToggleButton value="org" aria-label="organization">
            Organization
          </ToggleButton>
        ...
        {repoType === 'personal' && (
          <TextField
            label="Search Personal Repositories"
            placeholder="Type to search your personal repositories..."
            value={personalRepoSearch}
            onChange={(e) => {
              setPersonalRepoSearch(e.target.value);
              handleFilter(e.target.value);
            }}
            fullWidth
            sx={{ flex: 1 }}
            helperText="Type to filter your personal repositories"
          />
        )}
        ...
        <Button
          ...
          disabled={loadingPage || (repoType === 'org' && !orgName.trim())}
        >
          {repoType === 'personal' ? 'Load Repos' : 'Search'}
        </Button>
      </Box>
      ...
      {visible.length > 0 && (
        <>
          <Box display="flex" alignItems="center" gap={1}>
            <TextField
              placeholder={repoType === 'personal' ? 'Filter repositories' : 'Filter current page'}
              size="small"
