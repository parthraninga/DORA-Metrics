...
export const TeamPopover: FC<{
  ...
  const teamReposProdBranchMap = useSelector((s) => s.app.teamsProdBranchMap);
  const teamReposMaps = useSelector((s) => s.team.teamReposMaps);
  const repoCountForTeam = useCallback(
    (teamId: string) =>
      teamReposProdBranchMap[teamId]?.length ??
      teamReposMaps?.[teamId]?.length ??
      0,
    [teamReposProdBranchMap, teamReposMaps]
  );
