...
import { fetchTeams, fetchTeamsSupabase } from '@/slices/team';
...
export const useTeamSelectorSetup = ({ mode }: UseTeamSelectorSetupArgs) => {
  ...
  const teamReposMaps = useSelector((state) => state.team.teamReposMaps);
  ...
  useEffect(() => {
    if (orgId && integrationList.length) {
      fetchAllTeams();
      return;
    }
    if (!orgId) {
      dispatch(fetchTeamsSupabase());
    }
  }, [dispatch, fetchAllTeams, integrationList.length, orgId]);

  const dateRangeLabel = !partiallyUnselected
    ? `${format(dateRange[0], 'do MMM')} to ${format(dateRange[1], 'do MMM')}`
    : 'Select Date Range';
  const selectedTeamRepoCount =
    teams[0]?.id != null
      ? teamReposProdBranchMap?.[teams[0].id]?.length ??
        teamReposMaps?.[teams[0].id]?.length ??
        0
      : 0;
  const teamsLabel = teams.length
    ? !isSingleMode
      ? `${teams.length} ${pluralize('team', teams.length)} selected`
      : `${teams[0]?.name} (${selectedTeamRepoCount} ${pluralize(
          'repo',
          selectedTeamRepoCount
        )})`
