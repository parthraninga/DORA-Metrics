...
import { deleteTeam, deleteTeamSupabase, fetchTeams, fetchTeamsSupabase } from '@/slices/team';
...
const MoreOptions = ({ teamId }: { teamId: ID }) => {
  ...
  const handleTeamDeletion = useCallback(
    (teamId: ID) => {
      depFn(loading.true);
      const deleteAction = orgId
        ? dispatch(deleteTeam({ org_id: orgId, team_id: teamId }))
        : dispatch(deleteTeamSupabase({ team_id: teamId }));
      const refetch = orgId
