...
export const teamSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    addFetchCasesToReducer(builder, fetchTeamsSupabase, 'teams', (state, action) => {
      if (action.payload) {
        state.teams = action.payload.teams;
        state.teamReposMaps = action.payload.teamReposMap;
      }
...
// Supabase-backed teams (when no org / repos from DB)
type SupabaseTeamRow = {
  id: string;
  name: string;
  created_at: string;
  repo_ids: string[];
  repos: { id: string; repo_name: string; org_name: string }[];
  last_fetched_at?: string | null;
};

export const fetchTeamsSupabase = createAsyncThunk(
  'teams/fetchTeamsSupabase',
  async (_params, { rejectWithValue }) => {
    const list = await handleApi<SupabaseTeamRow[]>('/teams');
    const teams: Team[] = (list || []).map((t) => ({
      id: t.id,
      name: t.name,
      org_id: '',
      member_ids: [],
      created_at: typeof t.created_at === 'string' ? t.created_at : (t.created_at as unknown as Date)?.toISOString?.() ?? '',
      updated_at: typeof t.created_at === 'string' ? t.created_at : (t.created_at as unknown as Date)?.toISOString?.() ?? '',
      is_deleted: false,
      last_fetched_at: t.last_fetched_at ?? null,
    }));
    const teamReposMap: Record<ID, DB_OrgRepo[]> = {};
    (list || []).forEach((t) => {
      teamReposMap[t.id] = (t.repos || []).map((r) => ({
        id: r.id,
        org_id: '',
        name: r.repo_name,
        provider: 'github' as Integration,
        org_name: r.org_name || '',
        is_active: true,
        default_branch: 'main',
        language: '',
        ...
        slug: r.repo_name,
        deployment_type: 'WORKFLOW' as const,
        repo_workflows: []
      }));
    });
    return { teams, teamReposMap };
  }
);

export const createTeamSupabase = createAsyncThunk(
  'teams/createTeamSupabase',
  async (params: { name: string; repo_ids: string[] }) => {
    return await handleApi<{ id: string; name: string; created_at: string; repo_ids: string[] }>(
      '/teams',
...
export const updateTeamSupabase = createAsyncThunk(
  'teams/updateTeamSupabase',
  async (params: { team_id: ID; name?: string; repo_ids?: string[] }) => {
    return await handleApi<{ id: string; name: string; created_at: string; repo_ids: string[] }>(
      `/teams/${params.team_id}`,
      {
        method: 'PATCH',
        data: { name: params.name, repo_ids: params.repo_ids }
      }
    );
  }
);

export const deleteTeamSupabase = createAsyncThunk(
  'teams/deleteTeamSupabase',
  async (params: { team_id: ID }) => {
    return await handleApi<{ status: string }>(`/teams/${params.team_id}`, {
      method: 'DELETE'
    });
  }
);
