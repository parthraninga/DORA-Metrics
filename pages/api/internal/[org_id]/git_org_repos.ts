...
import {
  getGithubToken,
  getGitlabToken,
  getGithubTokenByIntegrationId,
  getGitlabTokenByIntegrationId,
  getGitHubGraphQLUrl,
  replaceURL,
...
const getSchema = yup.object({
  ...
  repo_type: yup.string().oneOf(['org', 'personal']).optional().default('org'),
  integration_id: yup.string().uuid().optional(),
})
...
export async function fetchRepos(params: {
  ...
  first?: number
  after?: string | null
  repo_type?: 'org' | 'personal'
}): Promise<{
  totalCount: number | null
  pageInfo: PageInfo | null
  repos: RepoItem[]
}> {
  const { provider, token, org, first = 50, after = null, repo_type = 'org' } = params
...
endpoint.handle.GET(getSchema, async (req, res) => {
  const { org_id, provider, org, first, after, select_all, repo_type = 'org', integration_id } = req.payload
  
  // Use integration_id if provided, otherwise fall back to org_id
  let token: string
  if (integration_id) {
    token =
      provider === Integration.GITHUB
        ? await getGithubTokenByIntegrationId(integration_id)
        : await getGitlabTokenByIntegrationId(integration_id)
  } else {
    token =
      provider === Integration.GITHUB
