...
export const getGithubTokenByIntegrationId = async (integration_id: ID) => {
  return await db('Integration')
    .select()
    .where({
      id: integration_id,
    ...
    .then((r) => {
      if (!r) {
        throw new Error(`Integration with id ${integration_id} not found`);
      }
      return dec(r.access_token_enc_chunks);
    });
};

export const getGitlabTokenByIntegrationId = async (integration_id: ID) => {
  return await db('Integration')
    .select()
    .where({
      id: integration_id,
