...
endpoint.handle.POST(postSchema, async (req, res) => {
  ...
  const { org_repos, org_id, name, integration_id } = req.payload;
  ...
  // If integration_id not provided, get default integration for the org
  let finalIntegrationId = integration_id;
  if (!finalIntegrationId) {
    const defaultIntegration = await db('Integration')
      .where('org_id', org_id)
      .orderBy('created_at', 'asc')
      .first();
    if (defaultIntegration) {
      finalIntegrationId = defaultIntegration.id;
    }
  }
  
  const [team, onboardingState] = await Promise.all([
    createTeam(org_id, name, [], finalIntegrationId),
...
endpoint.handle.PATCH(patchSchema, async (req, res) => {
  ...
  const { org_id, id, name, integration_id, org_repos } = req.payload;
  const orgReposList: ReqRepoWithProvider[] = [];
  forEachObjIndexed((repos, org) => {
    repos.forEach((repo) => {
      orgReposList.push({
  ...
  const [team] = await Promise.all([
    updateTeam(id, name, [], integration_id),
